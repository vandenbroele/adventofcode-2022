...
short memo[MAX_MINUTES + 1][MAX_VALVES][MAX_MASK];
...
int solve(int minute, valve *current_valve, int bitmask){
    if(minute == 0) return 0;
    
    if(memo[minute][current_valve->index][bitmask] == -1){
        int i, max_flow;
    
        max_flow = 0;
        if(!current_valve->released){
            max_flow = (minute - 1) * current_valve->flow_rate;
        }

        for(i = 0; i < current_valve->neighbour_count; i++){
            max_flow = max(max_flow, solve(minute - 1, current_valve->neighbours[i], bitmask));

            if(2 <= minute && !current_valve->released && current_valve->flow_rate != 0){
                current_valve->released = 1;
                max_flow = max(max_flow, ((minute - 1) * current_valve->flow_rate) + solve(minute - 2, current_valve->neighbours[i], bitmask + current_valve->bitmask));
                current_valve->released = 0;
            }
        }
        
        memo[minute][current_valve->index][bitmask] = max_flow;
    }
    
    return memo[minute][current_valve->index][bitmask];
}
