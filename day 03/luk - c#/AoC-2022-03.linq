...
//CrZsJsPPZsGzwwsLwLmpwMDw";
	...
	// ex 1
	sacks
		.Sum(s => s.OverlapValue)
		.Dump("ex 1");

	// ex 2
	var groups = new List<Group>();
	for (int i = 0; i < sacks.Length; i += 3)
		groups.Add(new Group { Sacks = sacks.Skip(i).Take(3).ToList() });
		
	groups
		.Select(g => CharValue(g.Badge))
		.Sum()
		.Dump("ex 2");
...
public class Group
{
	...
	public char Badge
	{
		get
		{
			var seed = Sacks.SelectMany(s => s.AllContent).ToList();
			return Sacks
				.Aggregate(seed, (l, s) => l.Intersect(s.AllContent).ToList())
				.Single();
		}
	}
}
...
public class Sack
{
	public Compartment A { get; set; }
	public Compartment B { get; set; }

	public char[] AllContent => A.Content.Concat(B.Content).ToArray();

	public char Overlap => A.Content.Intersect(B.Content).Single();

	public int OverlapValue => CharValue(Overlap);


	public static Sack Parse(string value)
	{
		var middle = value.Length / 2;

		return new Sack
		{
			A = Compartment.Parse(value.Substring(0, middle)),
			B = Compartment.Parse(value.Substring(middle)),
		};
	}
}
...
public class Compartment
{
	...
	public static Compartment Parse(string value)
	{
...
string input = @"ZNNvFWHqLNPZHHqPTHHnTGBhrrpjvmwfMmpfpjBjwpmw
