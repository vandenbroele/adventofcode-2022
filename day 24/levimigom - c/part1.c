...
void bfs(int rows, int cols, short states[MAX_ROWS][MAX_COLS]){
    ...
    for(i = 0; i < cols; i++){
        if(states[0][i] == empty){
            add_to_queue(queue, size, &items, 0, i, 1);
            break;
        }
    }
    ...
    for(i = 0; i < items && !(queue[i]->x == rows - 1 && states[queue[i]->x][queue[i]->y] == empty); i++){
