...
var answer2 = new String(stacks.Values.Select(x => x.Peek()).ToArray()).Dump("Answer 2");
...
void DoMoves(Dictionary<int, Stack<char>> stacks, List<(int crates, int from, int to)> moves)
{
	...
	{
		var buffer = new char[move.crates];
		
		for (var i = 0; i < move.crates; i++)
			buffer[i] = stacks[move.from].Pop();

		buffer = buffer.Reverse().ToArray();

		for (var i = 0; i < buffer.Length; i++)
			stacks[move.to].Push(buffer[i]);
	}
}
